...
| **3** | **`p_ffo`.** Mismo helper en `_populate_prescoring_metrics`. Fixture sintética de REIT extranjero que prueba el guard. | No (0 en los universos versionados) | `--compare` muestra 0 movidos; la fixture falla sin el guard; `make check` verde | ⏳ |
