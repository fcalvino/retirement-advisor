...
> **Rol:** `living-guide`. Esto es lo que **falta hacer**. Última repriorización: 2026-09-26 (orden acordado: TEST-NET → PIT-1; TEST-NET cerró el mismo día). Sincronizado el mismo día con los issues abiertos: la tabla de abiertas los nombra a todos.
>
...
| **PIT-1** | 2 | Medir los outcomes del backtesting point-in-time vía yfinance y escribir las 8 columnas de `synthetic_recommendation`. Bloquea a U5-1b. Alcance decidido el 2026-09-26 (ver bloque 3); **es lo próximo** (TEST-NET cerró) |
...
| **#149** BTC-SELL | 1 (provisoria) | El motor saca SELL para BTC en cualquier mercado: la fórmula cripto topea en 66 y BTC paga siempre 15 de drawdown y ≥ 8 de volatilidad, así que su mejor caso es 43 < `STRATEGY.reduce_score` (45) — CONTEXT §8. Banda 1 provisoria hasta ver en vivo si la ficha de Stock Analysis avisa que es aritmética; si avisa, baja a 4 |
