...
> **Rol:** `living-guide`. Esto es lo que **falta hacer**. Última repriorización: 2026-09-26 (orden acordado: TEST-NET → PIT-1; TEST-NET cerró el mismo día). Sincronizado el mismo día con los issues abiertos: la tabla de abiertas los nombra a todos. PIT-1 cerró el mismo día; la prueba en vivo confirmó #149 como banda 1, así que va antes que PIT-2.
>
...
| id | banda | qué |
|---|---|---|
| **U5-1b** | 3 | Recalibrar Piotroski vs moat. Bloqueado: n=11 orgánico a 30 días, o hasta que PIT-2 corra el volumen (PIT-1 ya escribe el outcome a 1 año; hoy no hay filas sintéticas) |
| **PIT-2** | 3 | Correr el volumen amplio y exponer la evidencia (F-Score vs retorno forward) en una lectura — hoy no la lee nadie. PIT-1 cerró: `scripts/score_synthetic_outcomes.py` ya escribe el outcome de cada fila. Va después de #149 |
