...
- [ ] 2. Backtesting point-in-time para evidencia sintética — **en curso, PR 3/N mergeado.** Acordado el alcance con el usuario antes de codear en cada PR (varias rondas de investigación conjunta: N6/N6c/U5-18d, `SecEdgarSource`, qué necesita realmente Piotroski vs el scorer completo, qué hacer con la mitad "moat" de U5-1b, y dónde vive el storage sintético). PR 1: [#100](https://github.com/fcalvino/retirement-advisor/pull/100) — `analysis/point_in_time.py`, reconstrucción de fundamentales SEC ancladas a `filed <= cutoff`. PR 2: [#102](https://github.com/fcalvino/retirement-advisor/pull/102) — `analysis/point_in_time_piotroski.py`, Piotroski F-Score point-in-time completo (9 checks) reusando `EnhancedScoring._piotroski_score` sin duplicar lógica; 4 rondas de code review encontraron y corrigieron bugs reales de alineación de fechas entre conceptos, contaminación del eje de períodos por instant facts y un fallback `info.get(...) or 0` en `analysis/scoring.py` que afectaba también al path de producción en vivo — corregido ahí para que todo caller lo reciba. PR 3: [#104](https://github.com/fcalvino/retirement-advisor/pull/104) — `analysis/synthetic_backtest.py`, storage separado (tabla + store + singleton propios, nunca `RecommendationLog`/`track_record_store`) con oráculo de aislamiento propio (mismo patrón que `tests/test_track_record_isolation_oracle.py`, verificado por mutación). Sigue sin generar volumen ni tocar la calibración. **Falta para cerrar la idea:** (a) generar volumen (correr `piotroski_as_of` sobre el universo × múltiples cutoffs históricos, persistir con `synthetic_backtest_store.log_piotroski`), (b) medir outcomes a 1 año vía precio yfinance (point-in-time-safe, no necesita reconstrucción) — requiere migrar la tabla (`_migrate`, mismo patrón que `track_record.py`) para agregar los campos de outcome, no diseñados todavía a propósito, (c) conectar esa evidencia a la calibración de `PiotroskiConfig.strong_threshold`/`bonus_strong`. La mitad "moat" de U5-1b queda fuera de esta idea (el tramo IA introduciría hindsight bias; el tramo cuantitativo 0-12 es un candidato de PR futuro, no comprometido todavía)
