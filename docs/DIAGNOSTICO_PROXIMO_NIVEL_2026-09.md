...
- [x] 2. Backtesting point-in-time para evidencia sintética — **cerrada, PR 5/N mergeado.** Acordado el alcance con el usuario antes de codear en cada PR (varias rondas de investigación conjunta: N6/N6c/U5-18d, `SecEdgarSource`, qué necesita realmente Piotroski vs el scorer completo, qué hacer con la mitad "moat" de U5-1b, dónde vive el storage sintético, y política de fair-access de SEC EDGAR confirmada en vivo). PR 1: [#100](https://github.com/fcalvino/retirement-advisor/pull/100) — `analysis/point_in_time.py`. PR 2: [#102](https://github.com/fcalvino/retirement-advisor/pull/102) — `analysis/point_in_time_piotroski.py`, Piotroski F-Score point-in-time completo (9 checks) reusando `EnhancedScoring._piotroski_score`; 4 rondas de review, incluido un fix real en `analysis/scoring.py` (F9) que también beneficia al path de producción en vivo. PR 3: [#104](https://github.com/fcalvino/retirement-advisor/pull/104) — `analysis/synthetic_backtest.py`, storage separado con oráculo de aislamiento propio. PR 4: [#106](https://github.com/fcalvino/retirement-advisor/pull/106) — `scripts/point_in_time_backtest.py`, genera volumen real vía SEC EDGAR (verificado end-to-end contra la API en vivo, no un mock). PR 5: [#108](https://github.com/fcalvino/retirement-advisor/pull/108) — **solo schema:** 8 columnas de outcome (`price_at_cutoff`, `price_at_horizon`, `horizon_date`, `return_pct`, `benchmark_return_pct`, `excess_return_pct`, `benchmark_missing`, `outcome_scored_at`) sobre `synthetic_recommendation`, ninguna se mide todavía; migración `_migrate_outcome_columns` con flag de verificación (`outcome_columns_verified`, chequeado por el batch script igual que `unique_index_verified`) y `server_default` alineado entre `create_all` y `ADD COLUMN`; **9 rondas de review** (varias por límite de sesión) que terminaron simplificando la migración de ~140 a ~55 líneas — reintento por batch, no por columna. **7 rondas de code review** en PR 4 encontraron y corrigieron bugs reales: alineación de fechas entre conceptos XBRL, contaminación del eje de períodos por instant facts, caché de CIK envenenado sin reintento real, condición de carrera en `SecEdgarSource._resolve_cik` (fix en `data/data_sources.py`, fuera del scope original pero necesario), conflación de "SEC confirma que no hay datos" con "falla real de red" (rompía el exit code contra `DEFAULT_TICKERS`, que trae ADRs/ETFs/crypto sin CIK), y conflación de "ya estaba logueado" con "la escritura falló" (mismo problema, en `log_piotroski`). `docs/CONTEXT.md` §6 actualizado con el estado real de la serie completa. **Motor funcional completo — calibración diferida.** Lo entregado (5 PRs, todo mergeado y testeado, verificado contra SEC EDGAR en vivo): dado un ticker y un corte histórico, reconstruir sus fundamentales como se veían en esa fecha sin look-ahead bias (#100), computar su F-Score Piotroski point-in-time de 9 checks (#102), a volumen (#106), en una tabla aislada de `track_record` con schema de outcome listo para llenar (#104, #108). Lo que NO se hizo, movido a `BACKLOG.md` como ítems independientes para no perderlo: **PIT-1** (medir los outcomes vía precio de yfinance y escribir las 8 columnas), **PIT-2** (correr el volumen sobre un universo amplio y exponer la evidencia en una lectura), y la recalibración de `PiotroskiConfig` en sí, que sigue siendo **U5-1b** — decisión humana explícita, nunca automática. El motor es el prerequisito de U5-1b y ahora está puesto; el payoff (sacar a `PiotroskiConfig`/bandas REIT del "percentile anchoring") queda a esa decisión de distancia. La mitad "moat" de U5-1b sigue fuera (el tramo IA introduciría hindsight bias; el cuantitativo 0-12 es candidato de PR futuro, no comprometido). PR 5: [#108](https://github.com/fcalvino/retirement-advisor/pull/108).
