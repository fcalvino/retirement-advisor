...
- [ ] 2. Backtesting point-in-time para evidencia sintética — **en curso, PR 1/N mergeado.** Acordado el alcance con el usuario antes de codear (dos rondas de investigación conjunta sobre N6/N6c/U5-18d y `SecEdgarSource`). PR 1: [#100](https://github.com/fcalvino/retirement-advisor/pull/100) — `analysis/point_in_time.py`, reconstrucción de fundamentales SEC ancladas a `filed <= cutoff` (sin scorer, sin persistencia, cero riesgo N6). **Falta para cerrar la idea:** (a) correr el scorer/`strategy.full_analysis` contra los fundamentales reconstruidos, (b) decidir e implementar dónde viven las recomendaciones sintéticas — recomendación ya documentada en la discusión del PR 1: tablas separadas en el mismo `DB_PATH`, nunca reusar `RecommendationLog`/`track_record_store`, con oráculo de aislamiento propio (mismo patrón que `tests/test_track_record_isolation_oracle.py`), (c) generar el volumen de recomendaciones sintéticas y conectarlas a la calibración de `StrategyConfig`/`PiotroskiConfig`/bandas REIT (U5-1b)
